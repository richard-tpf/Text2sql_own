...
使用 claude-opus-4-6 模型 + 双 Milvus 知识库。
启动后访问 http://localhost:8000
"""

from demo04.DWSystemPromptBuilder import DWSystemPromptBuilder
from demo04.LoggingLifecycleHook import LoggingLifecycleHook
from demo04.LoggingLlmMiddleware import LoggingLlmMiddleware
from demo04.LoggingObservabilityProvider import LoggingObservabilityProvider
from demo04.NoOpAgentMemory import NoOpAgentMemory
from demo04.knowledge_base import DualKnowledgeBaseManager
from demo04.knowledge_tools import SaveKnowledgeTool, SearchKnowledgeTool
from demo04.requirement_parser_tool import ParseRequirementDocTool
from demo04.generate_dws_sql_tool import GenerateDwsSqlTool
from demo04.generate_ads_sql_tool import GenerateAdsSqlTool
from demo04.TaskTitlePresearchWorkflowHandler import TaskTitlePresearchWorkflowHandler
from demo04.TimeoutLlmService import TimeoutLlmService
...
# ============================================================
# 1. 配置 LLM（connection test passed; change model only）
# ============================================================
_base_llm = OpenAILlmService(
    model="claude-opus-4-6",
    base_url="https://yibuapi.com/v1",
    api_key="sk-Ffxhvvk6z78RAeqUS9qq13RzeXe1rTUNMDGWNzRzXfZ1OT4C"  # TODO: 替换为你的 API Key
)

# 使用超时包装器，5 分钟（300 秒）超时自动终止会话
llm = TimeoutLlmService(
    llm_service=_base_llm,
    timeout_seconds=300,  # 5 分钟
    timeout_message="抱歉，LLM 响应超时（已超过 5 分钟）。当前会话已自动终止，请尝试简化您的问题或稍后重试。",
)
...
# 注册需求文档解析工具
tools.register_local_tool(ParseRequirementDocTool(), access_groups=['admin', 'user'])
# 注册 DWS 层 SQL 生成工具（传入 kb_manager 用于查询开发规范）
tools.register_local_tool(GenerateDwsSqlTool(kb_manager), access_groups=['admin', 'user'])
# 注册 ADS 层 SQL 生成工具（传入 kb_manager 用于查询开发规范）
tools.register_local_tool(GenerateAdsSqlTool(kb_manager), access_groups=['admin', 'user'])
...
agent = Agent(
    ...
    workflow_handler=TaskTitlePresearchWorkflowHandler(),
    config=agent_config,
...
from demo04.schema_upload_page import get_schema_upload_html
from demo04.business_upload_page import get_business_upload_html
from demo04.custom_index_page import get_custom_index_html
