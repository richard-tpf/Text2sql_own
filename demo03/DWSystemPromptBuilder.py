...
class DWSystemPromptBuilder(DefaultSystemPromptBuilder):
    ...
    async def build_system_prompt(
        ...
        has_parse_req = "parse_requirement_doc" in tool_names
        has_generate_dws = "generate_dws_sql" in tool_names
        has_generate_ads = "generate_ads_sql" in tool_names
        today_date = datetime.now().strftime("%Y-%m-%d")

        parts = []
        parts.append(self._build_role(today_date))
        parts.append(self._build_architecture())
        parts.append(self._build_dev_flow(has_parse_req, has_generate_dws, has_generate_ads))
        ...
        if has_generate_dws:
            parts.append(self._build_generate_dws_guide())

        if has_generate_ads:
            parts.append(self._build_generate_ads_guide())
...
- ADS 层为最终输出层"""
    ...
    def _build_dev_flow(self, has_parse_req: bool = False, has_generate_dws: bool = False, has_generate_ads: bool = False) -> str:
        parse_req_guide = ""
        if has_parse_req:
            parse_req_guide = """

【自动解析】当用户提供需求文档（Markdown 格式）时：
- 首先调用 parse_requirement_doc 工具，自动提取结构化的指标定义
- 工具会输出：需求概述、指标定义列表、涉及的表/字段清单
- 基于工具解析结果进行后续验证和开发文档编写，无需手动逐行提取"""

        generate_dws_guide = ""
        if has_generate_dws:
            generate_dws_guide = """
- 可调用 generate_dws_sql 工具，基于字段定义和来源表 DDL 自动生成规范化的 DWS 层 SQL"""

        generate_ads_guide = ""
        if has_generate_ads:
            generate_ads_guide = """
- 可调用 generate_ads_sql 工具，基于目标表名、时间粒度和来源 DWS 表自动生成规范化的 ADS 层 SQL"""
...
{generate_dws_guide}- 数据来源仅限 DWD 和 MID 层（以知识库中 layer 字段为准）
...
10. 周/月表聚合方式错误：可加指标用 SUM，不可加指标回源 DWD"""
    ...
    def _build_kb_workflow(self, has_search: bool, has_save: bool) -> str:
        ...
        if has_search:
            parts.extend([
                ...
                "",
                "【强制执行规则】",
                "当用户给出需求标题或任务名（如“开发XX指标统计表”）时，必须先调用 search_knowledge(search_scope='business') 检索需求文档。",
                "只有在检索结果明确为空时，才允许向用户索要补充文档；禁止在未检索前直接要求用户提供文档。",
            ])
    ...
    def _build_generate_dws_guide(self) -> str:
        return """
============================================================
DWS 层 SQL 生成工具使用指南
============================================================

调用 generate_dws_sql 工具时需提供以下参数：

1. table_name: 目标 DWS 表名，必须符合命名规范 zijie.dws_<主题域>_<业务描述>_<时间粒度>_df

2. fields: 字段定义列表，每个字段包含：
   - name: 字段名（snake_case）
   - data_type: 数据类型（STRING/BIGINT/DECIMAL(18,2)等）
   - comment: 中文注释
   - source_field: 来源字段表达式（如 'a.user_id' 或 'CASE WHEN ... END'）
   - aggregate_type: 聚合类型（SUM/COUNT/COUNT_DISTINCT/MAX/MIN/AVG/NONE）
   - is_dimension: 是否为维度字段（GROUP BY 字段）
   - nullable_handling: 是否使用 COALESCE 处理空值

3. source_ddls: 来源表 DDL 列表（从知识库搜索获取）

4. source_aliases: 来源表别名列表，与 source_ddls 一一对应（如 ['a', 'b']）

5. process_pattern: 加工模式
   - direct_aggregate: 直接聚合（无需打标，直接 GROUP BY 聚合）
   - single_cte_flag: 单 CTE flag（单来源表打标后聚合）
   - multi_cte_flag: 多 CTE 打标（多个来源表分别打标后聚合）

6. join_conditions: 表关联条件列表（多表场景必填）
   - left_table_alias / right_table_alias: 左/右表别名
   - join_type: 关联类型（LEFT JOIN / INNER JOIN）
   - on_condition: 关联条件表达式

加工模式选择指南：
- 单表直接聚合统计 → direct_aggregate
- 单表需要多个 CASE WHEN 打标 → single_cte_flag
- 多表 JOIN 后聚合 → multi_cte_flag"""

    def _build_generate_ads_guide(self) -> str:
        return """
============================================================
ADS 层 SQL 生成工具使用指南
============================================================

调用 generate_ads_sql 工具时需提供以下参数：

1. table_name: 目标 ADS 表名，必须符合命名规范 zijie.ads_<主题域>_<业务描述>_<时间粒度>_df

2. time_granularity: 时间粒度
   - day: 日粒度
   - week: 周粒度（按周一聚合）
   - month: 月粒度（按月份聚合）

3. dws_table: 来源 DWS 表名（如 zijie.dws_credit_user_stats_df）

4. fields: 字段定义列表（可选，不填则生成模板），每个字段包含：
   - name: 字段名（snake_case）
   - data_type: 数据类型（STRING/BIGINT/DECIMAL(18,2)等）
   - comment: 中文注释
   - source_field: 来源字段表达式（如 'dws.user_id'）
   - aggregate_type: 聚合类型（SUM/COUNT/COUNT_DISTINCT/MAX/MIN/AVG/NONE）
   - is_dimension: 是否为维度字段（GROUP BY 字段）
   - nullable_handling: 是否使用 COALESCE 处理空值

5. where_conditions: WHERE 过滤条件列表（可选）
...
- ADS 层只引用 DWS 层数据
- 派生指标（比率类）在 ADS 层计算
- 不可加指标（COUNT DISTINCT）周/月表需回源 DWD 重新计算"""
