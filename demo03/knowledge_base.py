...
class MilvusSchemaKnowledgeBase:
    ...
    def _build_bilingual_text(table_name: str, ddl: str) -> str:
        ...
        优化策略：英文表名和字段名优先，重复出现以提高匹配权重。
        格式：`表名 表名(不含schema) 字段名列表 表名 表级注释 字段名:中文注释...`
        使得英文搜索（如"dwd_fact_credit_application_df"）优先命中，
        同时中文搜索（如"授信申请事实表"）也能命中。
        """
        parts = []
        ...
        if all_field_names:
            parts.extend(all_field_names)  # 先加所有英文字段名
